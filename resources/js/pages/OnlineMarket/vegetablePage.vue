      ...
      <div class="max-w-7xl mx-auto px-4 space-y-8">
        ...
        >
          ...
          <div class="relative flex items-end h-full p-6 md:p-8">
            <h2 class="text-4xl md:text-5xl font-bold text-white drop-shadow-lg">VEGETABLES</h2>
          </div>
        </div>

        <!-- Search Results Info -->
        <div v-if="searchQuery" class="bg-green-50 border-l-4 border-green-500 p-4 rounded">
          <p class="text-gray-700">Showing results for: <span class="font-semibold text-green-600">"{{ searchQuery }}"</span></p>
        ...
        <div v-else class="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p class="text-gray-500 text-lg mb-4">
            {{ searchQuery ? `No vegetables found for "${searchQuery}".` : 'No vegetables available at the moment.' }}
          </p>
          <button
            @click="clearSearch"
            v-if="searchQuery"
