                ...
                <table class="w-full text-sm text-left table-auto">
                    <thead>
                        <tr class="border-b border-gray-200 font-semibold text-gray-700">
                            ...
                            <th class="py-2 px-2 cursor-pointer">Customer Name<span>{{ getSortIcon('full_name') }}</span></th>
                        ...
                        <tr v-for="order in paginatedOrders" :key="order.id" class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="py-2 px-2">
                                <div v-for="item in order.order_items" :key="item.id">
                                    <img :src="getProductImage(item.product?.image)" class="w-10 h-10 object-cover rounded mb-1" />
                                </div>
                            </td>
                            <td class="py-2 px-2">
                                <div v-for="item in order.order_items" :key="item.id">
                            ...
                            <td class="py-2 px-2">
                                <div v-for="item in order.order_items" :key="item.id">
                                    ₱{{ item.product ? item.product.price : 'N/A' }} <span class="text-xs text-gray-500">{{ item.product ? item.product.unit : '' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="py-2 px-2 space-x-2">
                                <button @click="viewOrder(order)" class="bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-medium py-1 px-3 rounded">
                                    View
                                </button>
                                <button v-if="isPending(order.status)" @click="markAsPaid(order)" class="bg-green-500 hover:bg-green-600 text-white text-xs font-medium py-1 px-3 rounded">
                                    Mark Paid
                                </button>
                                <button v-if="isPending(order.status)" @click="deleteOrder(order)" class="bg-red-500 hover:bg-red-600 text-white text-xs font-medium py-1 px-3 rounded">
                                    Delete
                                </button>
                            </td>
                        </tr>
                ...
                <div class="relative bg-white p-6 rounded-xl shadow-2xl w-full max-w-md mx-auto z-10">
                    ...
                    <div v-if="viewOrderData">
                        ...
                        <div class="mb-2">
                            ...
                            <span :class="getStatusClass(viewOrderData.status)">
                                {{ viewOrderData.status }}
                            </span>
                        </div>
                        <div>
                            <div v-for="item in viewOrderData.order_items" :key="item.id" class="flex items-center mb-4 space-x-3 border-b pb-3">
                                <img :src="getProductImage(item.product?.image)" class="w-16 h-16 rounded object-cover border" />
                                <div>
                    ...
                    <div class="flex justify-end mt-4">
                        <button @click="closeViewModal" class="bg-indigo-500 text-white px-4 py-2 rounded">Close</button>
                    </div>
                </div>
            </div>
